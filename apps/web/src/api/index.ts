...
/**
 * 绑定到某个聊天窗口的流式请求配置：
 * 流式 token 追加到该窗口消息，开始/结束/出错时切换 loading。
 * 原先在 chatMindmap.ts / file.ts 各有一份重复实现，现统一于此。
 */
export function createChatWindowOptions(url: string, chatWindowId: string, data: any): ChatOptions {
